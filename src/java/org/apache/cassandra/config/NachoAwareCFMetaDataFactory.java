...
public class NachoAwareCFMetaDataFactory extends CFMetaDataFactory
{
    ...
    {
        CFMetaData cfm = new NachoCFMetaData(keyspace,
                                             name,
                                             cfId,
                                             isSuper,
                                             isCounter,
        ...
        if (keyspace.equals(SystemKeyspace.NAME) && name.equals(SchemaKeyspace.TABLES))
        {
            ColumnDefinition def = ColumnDefinition.regularDef(cfm, UTF8Type.instance.fromString(COLUMN), UTF8Type.instance);
    ...
    @Override
    public void addTableToSchemaMutation(RowUpdateBuilder adder, CFMetaData table)
    {
        super.addTableToSchemaMutation(adder, table);
    ...
    @Override
    public CFMetaData createTableFromTableRowAndColumns(UntypedResultSet.Row row, List<ColumnDefinition> columns)
    {
        NachoCFMetaData cfm = (NachoCFMetaData) super.createTableFromTableRowAndColumns(row, columns);
        if (row.has(COLUMN))
        {
            Map<String, String> nachOptions = FBUtilities.fromJsonMap(row.getString(COLUMN));
