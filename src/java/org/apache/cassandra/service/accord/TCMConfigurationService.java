...
public class TCMConfigurationService implements ChangeListener
{
    ...
    public static List<Shard> createShards(KeyspaceMetadata keyspace, ClusterMetadata metadata)
    {
        ReplicationParams replication = keyspace.params.replication;
        Directory directory = metadata.directory;
        DataPlacement placement = metadata.placements.get(replication);
    ...
    static Topology clusterMetadataToTopology(ClusterMetadata metadata)
    {
        List<Shard> shards = new ArrayList<>();
        for (KeyspaceMetadata keyspace : metadata.schema.getKeyspaces())
            shards.addAll(createShards(keyspace, metadata));
        return new Topology(metadata.epoch.getEpoch(), shards.toArray(new Shard[0]));
    }
    ...
    public void notifyPostCommit(ClusterMetadata prev, ClusterMetadata next)
    {
        ...
        ChangeListener.super.notifyPostCommit(prev, next);
    }
}
