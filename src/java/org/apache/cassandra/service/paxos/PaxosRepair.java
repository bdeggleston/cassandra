...
public class PaxosRepair extends AbstractPaxosRepair
{
    ...
    private class Querying extends State implements RequestCallbackWithFailure<Response>, Runnable
    {
        ...
        private State execute()
        {
            ...
            boolean reproposalMayBeRejected     = clashingPromise != null || !isAfter(latestWitnessed, prevSupersededBy);
            ...
            {
                Ballot ballot = staleBallotNewerThan(latest(latestWitnessed, prevSupersededBy), paxosConsistency);
        ...
        public void run()
        {
            Message<Request> message = Message.out(PAXOS2_REPAIR_REQ, new Request(partitionKey(), table))
                                          .maybePermitArtificialLatency(participants.consistencyForConsensus);
