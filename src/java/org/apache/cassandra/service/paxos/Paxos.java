...
import static org.apache.cassandra.config.Config.PaxosVariant.v2_without_linearizable_reads_or_failed_writes;
...
public class Paxos
{
    ...
    private static final CassandraVersion MODERN_PAXOS_RELEASE = new CassandraVersion(System.getProperty("cassandra.paxos.modern_release", "4.1.0"));
    ...
    static class MaybeFailure
    {
        ...
        {
            this(contacted - failures.size() < required, contacted, required, successes, failures);
        }
            ...
            throws UnavailableException, IsBootstrappingException, RequestFailureException, RequestTimeoutException, InvalidRequestException
    {
        ...
        return cas(key, request, consistencyForConsensus, consistencyForCommit, start, proposeDeadline, commitDeadline);
    }
    public static RowIterator cas(DecoratedKey key,
                                  CASRequest request,
                                  ConsistencyLevel consistencyForConsensus,
            ...
            throws UnavailableException, IsBootstrappingException, RequestFailureException, RequestTimeoutException, InvalidRequestException
    {
        return cas(key, request, consistencyForConsensus, consistencyForCommit, nanoTime(), proposeDeadline, commitDeadline);
    }
            ...
            throws UnavailableException, IsBootstrappingException, RequestFailureException, RequestTimeoutException, InvalidRequestException
    {
        ...
        try (PaxosOperationLock lock = PaxosState.lock(partitionKey, metadata, proposeDeadline, consistencyForConsensus, true))
        {
            ...
            done: while (true)
            {
                ...
                if (!conditionMet)
                {
                    if (getPaxosVariant() == v2_without_linearizable_reads_or_failed_writes)
                    {
                ...
                else if (begin.isPromised)
                {
                    ...
                    PartitionUpdate updates = request.makeUpdates(current, begin.ballot);
                        ...
                        switch (propose.superseded().hadSideEffects)
                        {
                            ...
                            case NO:
                                ...
                                if (!waitForContention(proposeDeadline, ++failedAttemptsDueToContention, metadata, partitionKey, consistencyForConsensus, WRITE))
                                    throw new MaybeFailure(participants, 0, emptyMap()).markAndThrowAsTimeoutOrFailure(true, consistencyForConsensus, failedAttemptsDueToContention);
                        }
                    }
                }
                // continue to retry
        ...
        try (PaxosOperationLock lock = PaxosState.lock(read.partitionKey(), read.metadata(), deadline, consistencyForConsensus, false))
        {
            while (true)
            {
                ...
                switch (PAXOS_VARIANT)
                {
                    ...
                    case v2_without_linearizable_reads_or_failed_writes:
                        ...
                        switch (propose.superseded().hadSideEffects)
                        {
                            ...
                            case NO:
                                ...
                                if (!waitForContention(deadline, ++failedAttemptsDueToContention, group.metadata(), group.queries.get(0).partitionKey(), consistencyForConsensus, READ))
                                    throw new MaybeFailure(begin.participants, 0, emptyMap()).markAndThrowAsTimeoutOrFailure(true, consistencyForConsensus, failedAttemptsDueToContention);
                        }
                }
            }
        }
            ...
            retry: switch (prepare.outcome)
            {
                ...
                case SUPERSEDED:
                {
                    ...
                    if (!waitForContention(deadline, ++failedAttemptsDueToContention, query.metadata(), query.partitionKey(), consistencyForConsensus, isWrite ? WRITE : READ))
                        throw new MaybeFailure(prepare.participants, 0, emptyMap()).markAndThrowAsTimeoutOrFailure(true, consistencyForConsensus, failedAttemptsDueToContention);
    ...
    {
        return consistency == SERIAL || consistency == UNSAFE_DELAY_SERIAL ? GLOBAL : LOCAL;
    }
    ...
    public static boolean useV2()
    {
        switch (PAXOS_VARIANT)
        {
            ...
            case v2:
                return true;
            case v1:
            case v1_without_linearizable_reads:
                return false;
            default:
