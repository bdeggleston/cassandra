...
public class BlockingPartitionRepair extends AbstractFuture<Object> implements IAsyncCallback
{
    private final Keyspace keyspace;
    private final DecoratedKey key;
    private final ConsistencyLevel consistency;
    private final InetAddressAndPort[] participants;
    private final Map<InetAddressAndPort, PartitionUpdate> pendingRepairs;
    private final CountDownLatch latch;

    private volatile long mutationsSentTime;

    public BlockingPartitionRepair(Keyspace keyspace, DecoratedKey key, ConsistencyLevel consistency, Map<InetAddressAndPort, PartitionUpdate> repairs, int maxBlockFor, InetAddressAndPort[] participants)
    {
    ...
    private PartitionUpdate mergeUnackedUpdates()
    {
        ...
        List<PartitionUpdate> updates = Lists.newArrayList(pendingRepairs.values());
    ...
    public void sendInitialRepairs()
    {
        ...
        for (Map.Entry<InetAddressAndPort, PartitionUpdate> entry: pendingRepairs.entrySet())
        {
            InetAddressAndPort destination = entry.getKey();
            PartitionUpdate update = entry.getValue();
            TableId tid = update.metadata().id;

            Mutation mutation = ReadRepairs.createRepairMutation(update, consistency, destination, false);
            if (mutation != null)
            {
                Tracing.trace("Sending read-repair-mutation to {}", destination);
                // use a separate verb here to avoid writing hints on timeouts
                sendRR(mutation.createMessage(MessagingService.Verb.READ_REPAIR), destination);
                ColumnFamilyStore.metricsFor(tid).readRepairRequests.mark();
            }
    ...
    public void maybeSendAdditionalRepairs(long timeout, TimeUnit timeoutUnit)
    {
        ...
        for (InetAddressAndPort endpoint: candidates)
        {
            ...
            if (mutation == null)
            {
                mutation = ReadRepairs.createRepairMutation(update, consistency, endpoint, true);
    ...
    {
        return ReadRepairs.getCandidateEndpoints(keyspace, key, consistency);
    }
}
