...
public class ReadReconciliations implements ExpiredStatePurger.Expireable
{
    ...
    {
        // compute the summary
        MutationSummary summary = request.command.createMutationSummary(false);

        // send the summary to all peers, so that they can initiate reconciling their gaps ASAP
        Message<TrackedSummaryResponse> message = Message.out(Verb.TRACKED_SUMMARY_RSP, request.makeResponse(summary));
        if (request.dataNode != LOCAL_NODE)
            MessagingService.instance().send(message, host(request.dataNode));
        for (int node : request.summaryNodes)
            if (node != LOCAL_NODE)
                MessagingService.instance().send(message, host(node));

        // process the summary locally
    ...
    private static final class Coordinator
    {
        ...
        private boolean updateRemainingAndMaybeComplete(int summariesDelta, int mutationsDelta, int syncAcksDelta)
        {
