...
public class ViewUpdateGenerator
{
    ...
    private UpdateAction updateAction(Row existingBaseRow, Row mergedBaseRow)
    {
        ...
        return baseColumn.cellValueType().compare(ByteBuffer.wrap(before.value()), ByteBuffer.wrap(after.value())) == 0  // FIXME?
    ...
    private ByteBuffer getValueForPK(ColumnMetadata column, Row row)
    {
        switch (column.kind)
        {
            ...
            default:
                ...
                return ByteBuffer.wrap(row.getCell(column).value());
        }
    }
}
