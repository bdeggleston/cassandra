    ...
    public static class InValue extends Term.Terminal
    {
        ...
        public static InValue fromSerialized(ByteBuffer value, ListType type, QueryOptions options) throws InvalidRequestException
        {
            try
            {
                ...
                List<?> l = type.getSerializer().deserializeForNativeProtocol(value, ByteBufferHandle.instance, options.getProtocolVersion());
                ...
                for (Object element : l)
                    elements.add(Arrays.asList(tupleType.split(type.getElementsType().decomposeBuffer(element))));
