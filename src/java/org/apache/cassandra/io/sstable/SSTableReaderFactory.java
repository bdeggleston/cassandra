...
public class SSTableReaderFactory
{
    ...
    public SSTableReaderFactory(DatabaseDescriptor databaseDescriptor, SystemKeyspace systemKeyspace, Schema schema,
                                CacheService cacheService, ClusterState clusterState, Tracing tracing)
    {
    ...
    public SSTableReader open(Descriptor descriptor) throws IOException
    {
        ...
        if (descriptor.cfname.contains(SECONDARY_INDEX_NAME_SEPARATOR))
        {
            ...
            CFMetaData parent = schema.getCFMetaData(descriptor.ksname, parentName);
            ColumnDefinition def = parent.getColumnDefinitionForIndex(descriptor.cfname.substring(i + 1));
            metadata = CFMetaData.newIndexMetadata(parent, def, SecondaryIndex.getIndexComparator(parent, def));
        }
        else
        {
            metadata = schema.getCFMetaData(descriptor.ksname, descriptor.cfname);
        }
    ...
    {
        return open(descriptor, components, metadata, clusterState.getPartitioner(), false);
    }
