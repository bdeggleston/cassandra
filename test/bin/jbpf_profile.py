...
# BDE: copied from https://github.com/iovisor/bcc/blob/master/tools/profile.py,
# but modified to dump JIT symbols immediately before stack parsing
...
thread_group.add_argument("-p", "--pid", type=positive_int,
                          # required=True,  #BDE: we need this for perf-map-agent
...
# BDE: so we can dump symbols immediately before parsing the output
parser.add_argument("-J", "--perf-map-agent", dest='perf_map_agent', type=str, default="/Users/beggleston/perf-map-agent",
                    help="path to perf map agent (clone from https://github.com/jvm-profiling-tools/perf-map-agent and build)")
parser.add_argument("-I", "--include-interpreter", dest="include_interpreter", action="store_true",
                    help="include jvm interpreter frames (excluded by default)")
...
def maybe_filter_interpreter(stack):
    if args.include_interpreter:
        return stack
    else:
        return [s for s in stack if s != "Interpreter"]
...
if args.pid > -1:
    ...
    perf_map = args.perf_map_agent + '/bin/create-java-perf-map.sh'

    try:
        check_call([perf_map, str(args.pid)])
    except:
        print (args.perf_map_agent)
        print(perf_map)
        raise
    ...
    if args.folded:
        ...
        if not args.user_stacks_only:
            ...
            if stack_id_err(k.kernel_stack_id):
                ...
                line.extend(maybe_filter_interpreter([aksym(addr) for addr in reversed(kernel_stack)]))

        key = b";".join(line).decode('utf-8', 'replace')
        val = int(v.value)
        stats[key] = val if key not in stats else val + stats[key]
