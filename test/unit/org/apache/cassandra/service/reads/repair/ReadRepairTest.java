...
public class ReadRepairTest
{
    ...
    private static class InstrumentedPartitionRepair extends BlockingPartitionRepair
    {
        public InstrumentedPartitionRepair(Keyspace keyspace, DecoratedKey key, ConsistencyLevel consistency, Map<InetAddressAndPort, PartitionUpdate> repairs, int maxBlockFor, InetAddressAndPort[] participants)
        {
    ...
    static Cell cell2;
    static Cell cell3;
    static PartitionUpdate resolved;
    ...
    private static PartitionUpdate update(Cell... cells)
    {
    ...
    private static InstrumentedPartitionRepair createRepairHandler(Map<InetAddressAndPort, PartitionUpdate> updates, int maxBlockFor, Collection<InetAddressAndPort> participants)
    {
        InetAddressAndPort[] participantArray = new InetAddressAndPort[participants.size()];
        participants.toArray(participantArray);
        return new InstrumentedPartitionRepair(ks, key, ConsistencyLevel.LOCAL_QUORUM, updates, maxBlockFor, participantArray);
    }

    private static InstrumentedPartitionRepair createRepairHandler(Map<InetAddressAndPort, PartitionUpdate> updates, int maxBlockFor)
    {
        return createRepairHandler(updates, maxBlockFor, updates.keySet());
    }
    ...
    private static void assertUpdateEqual(PartitionUpdate expected, PartitionUpdate actual)
    {
        Assert.assertEquals(expected.metadata().id, actual.metadata().id);
        Assert.assertEquals(expected.partitionKey(), actual.partitionKey());
        assertRowsEqual(Iterables.getOnlyElement(expected), Iterables.getOnlyElement(actual));
    }

    private static void assertUpdateEqual(PartitionUpdate expected, Mutation actualMutation)
    {
        PartitionUpdate actual = Iterables.getOnlyElement(actualMutation.getPartitionUpdates());
        assertUpdateEqual(expected, actual);
    }
    ...
    public void additionalMutationRequired() throws Exception
    {
        ...
        PartitionUpdate repair1 = update(cell2);
        PartitionUpdate repair2 = update(cell1);

        // check that the correct repairs are calculated
        Map<InetAddressAndPort, PartitionUpdate> repairs = new HashMap<>();
        ...
        assertUpdateEqual(repair1, handler.mutationsSent.get(target1));
        assertUpdateEqual(repair2, handler.mutationsSent.get(target2));

        // check that a combined update is speculatively sent to the 3rd target
        ...
        assertUpdateEqual(resolved, handler.mutationsSent.get(target3));
    ...
    {
        Map<InetAddressAndPort, PartitionUpdate> repairs = new HashMap<>();
        repairs.put(target1, update(cell2));
        repairs.put(target2, update(cell1));
    ...
    public void mutationsArentSentToInSyncNodes() throws Exception
    {
        ...
        InstrumentedPartitionRepair handler = createRepairHandler(repairs, 2, participants);
        handler.sendInitialRepairs();
        Assert.assertEquals(1, handler.mutationsSent.size());
    ...
    {
        Map<InetAddressAndPort, PartitionUpdate> repairs = new HashMap<>();
        repairs.put(target1, update(cell1));
        repairs.put(target2, update(cell2));
        repairs.put(target3, update(cell3));
    ...
    public void remoteDCTest() throws Exception
    {
        ...
        repairs.put(remote1, update(cell1));
