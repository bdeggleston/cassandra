...
public class StreamingTransferTest
{
    ...
    {
        StreamResultFuture futureResult = new StreamPlan("StreamingTransferTest", DatabaseDescriptor.createMain(false), databaseDescriptor.getSchema(),
    ...
    private List<String> createAndTransfer(ColumnFamilyStore cfs, Mutator mutator, boolean transferSSTables) throws Exception
    {
        ...
        List<Row> rows = Util.getRangeSlice(cfs, DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing());
        assertEquals(offs.length, rows.size());
        for (int i = 0; i < offs.length; i++)
        {
            String key = "key" + offs[i];
            String col = "col" + offs[i];
            assert cfs.getColumnFamily(QueryFilter.getIdentityFilter(Util.dk(key, databaseDescriptor), cfs.name, System.currentTimeMillis(), DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing(), databaseDescriptor.getDBConfig())) != null;
    ...
    private void transferRanges(ColumnFamilyStore cfs) throws Exception
    {
        ...
        new StreamPlan("StreamingTransferTest", DatabaseDescriptor.createMain(false), databaseDescriptor.getSchema(),
    ...
    public void testTransferRangeTombstones() throws Exception
    {
        ...
        rm.add(cfname, cellname(1), ByteBuffer.wrap(new byte[DatabaseDescriptor.createMain(false).getColumnIndexSize() - 64]), 2);
        rm.add(cfname, cellname(6), ByteBuffer.wrap(new byte[DatabaseDescriptor.createMain(false).getColumnIndexSize()]), 2);
    ...
    public void testTransferTableMultiple() throws Exception
    {
        ...
        List<Row> rows = Util.getRangeSlice(cfstore, DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing());
        ...
        assert cfstore.getColumnFamily(QueryFilter.getIdentityFilter(Util.dk("transfer1", databaseDescriptor), "Standard1", System.currentTimeMillis(), DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing(), databaseDescriptor.getDBConfig())) == null;
        assert cfstore.getColumnFamily(QueryFilter.getIdentityFilter(Util.dk("transfer2", databaseDescriptor), "Standard1", System.currentTimeMillis(), DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing(), databaseDescriptor.getDBConfig())) == null;
        assert cfstore.getColumnFamily(QueryFilter.getIdentityFilter(Util.dk("test2", databaseDescriptor), "Standard1", System.currentTimeMillis(), DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing(), databaseDescriptor.getDBConfig())) == null;
        assert cfstore.getColumnFamily(QueryFilter.getIdentityFilter(Util.dk("test3", databaseDescriptor), "Standard1", System.currentTimeMillis(), DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing(), databaseDescriptor.getDBConfig())) == null;
    }
    ...
    public void testTransferOfMultipleColumnFamilies() throws Exception
    {
        ...
        for (Map.Entry<DecoratedKey,String> entry : Arrays.asList(first, last))
        {
            ...
            List<Row> rows = Util.getRangeSlice(store, DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing());
    ...
    public void testRandomSSTableTransfer() throws Exception
    {
        ...
        transfer(sstable, ranges);
        assertEquals(1, cfs.getSSTables().size());
        assertEquals(7, Util.getRangeSlice(cfs, DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing()).size());
    }
