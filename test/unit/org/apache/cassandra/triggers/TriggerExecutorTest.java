...
public class TriggerExecutorTest
{
    ...
    public void sameKeySameCfColumnFamilies() throws ConfigurationException, InvalidRequestException
    {
        ...
        ColumnFamily mutated = DatabaseDescriptor.createMain(false, false).getTriggerExecutor().execute(bytes("k1"), makeCf(metadata, "v1", null));
    ...
    public void sameKeyDifferentCfColumnFamilies() throws ConfigurationException, InvalidRequestException
    {
        ...
        DatabaseDescriptor.createMain(false, false).getTriggerExecutor().execute(bytes("k1"), makeCf(metadata, "v1", null));
    }
    ...
    public void noTriggerMutations() throws ConfigurationException, InvalidRequestException
    {
        ...
        assertNull(DatabaseDescriptor.createMain(false, false).getTriggerExecutor().execute(Collections.singletonList(rm)));
    }
    ...
    public void sameKeySameCfRowMutations() throws ConfigurationException, InvalidRequestException
    {
        ...
        List<? extends IMutation> tmutations = new ArrayList<>(DatabaseDescriptor.createMain(false, false).getTriggerExecutor().execute(Arrays.asList(rm1, rm2)));
    ...
    public void differentKeyRowMutations() throws ConfigurationException, InvalidRequestException
    {
        ...
        List<? extends IMutation> tmutations = new ArrayList<>(DatabaseDescriptor.createMain(false, false).getTriggerExecutor().execute(Arrays.asList(rm)));
        assertEquals(2, tmutations.size());
        Collections.sort(tmutations, new RmComparator());
    ...
    {
        ColumnFamily cf = ArrayBackedSortedColumns.factory.create(metadata, databaseDescriptor.getDBConfig());
    ...
    public static class SameKeySameCfTrigger implements ITrigger
    {
        public Collection<Mutation> augment(ByteBuffer key, ColumnFamily update)
        {
            ColumnFamily cf = ArrayBackedSortedColumns.factory.create(update.metadata(), databaseDescriptor.getDBConfig());
            cf.addColumn(new BufferCell(getColumnName(update.metadata(), "c2"), bytes("trigger")));
            return Collections.singletonList(databaseDescriptor.getMutationFactory().create(update.metadata().ksName, key, cf));
        }
    }
    ...
    public static class SameKeyDifferentCfTrigger implements ITrigger
    {
        public Collection<Mutation> augment(ByteBuffer key, ColumnFamily update)
        {
            ColumnFamily cf = ArrayBackedSortedColumns.factory.create(makeCfMetaData(update.metadata().ksName, "otherCf", null), databaseDescriptor.getDBConfig());
            cf.addColumn(new BufferCell(getColumnName(update.metadata(), "c2"), bytes("trigger")));
            return Collections.singletonList(databaseDescriptor.getMutationFactory().create(cf.metadata().ksName, key, cf));
        }
    }
    ...
    public static class SameKeyDifferentKsTrigger implements ITrigger
    {
        public Collection<Mutation> augment(ByteBuffer key, ColumnFamily update)
        {
            ColumnFamily cf = ArrayBackedSortedColumns.factory.create(makeCfMetaData("otherKs", "otherCf", null), databaseDescriptor.getDBConfig());
            cf.addColumn(new BufferCell(getColumnName(update.metadata(), "c2"), bytes("trigger")));
            return Collections.singletonList(databaseDescriptor.getMutationFactory().create(cf.metadata().ksName, key, cf));
        }
    }
    ...
    public static class DifferentKeyTrigger implements ITrigger
    {
        public Collection<Mutation> augment(ByteBuffer key, ColumnFamily update)
        {
            ColumnFamily cf = ArrayBackedSortedColumns.factory.create(update.metadata(), databaseDescriptor.getDBConfig());
            cf.addColumn(new BufferCell(getColumnName(update.metadata(), "c2"), bytes("trigger")));
            return Collections.singletonList(databaseDescriptor.getMutationFactory().create(cf.metadata().ksName, bytes("otherKey"), cf));
        }
    }
