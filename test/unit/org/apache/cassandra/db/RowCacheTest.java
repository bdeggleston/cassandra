...
public class RowCacheTest
{
    ...
    public void testRowCacheRange()
    {
        ...
        RowCacheKey rck = new RowCacheKey(cachedStore.metadata.cfId, dk, databaseDescriptor.getSchema());
        Mutation mutation = databaseDescriptor.getMutationFactory().create(KEYSPACE_CACHED, key);
        for (int i = 0; i < 200; i++)
