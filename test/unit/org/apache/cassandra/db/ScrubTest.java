...
public class ScrubTest
{
    ...
    public static void defineSchema() throws ConfigurationException
    {
        ...
        SchemaLoader.createKeyspace(KEYSPACE,
                                    ...
                                    databaseDescriptor.getCFMetaDataFactory().denseCFMetaData(KEYSPACE, COUNTER_CF, BytesType.instance).defaultValidator(CounterColumnType.instance),
    ...
    public void testScrubOneRow() throws ExecutionException, InterruptedException
    {
        ...
        fillCF(cfs, 1);
        rows = cfs.getRangeSlice(Util.range("", "", databaseDescriptor), null, new IdentityQueryFilter(DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing(), databaseDescriptor.getDBConfig()), 1000);
    ...
    public void testScrubCorruptedCounterRow() throws IOException, WriteTimeoutException
    {
        ...
        List<Row> rows = cfs.getRangeSlice(Util.range("", "", databaseDescriptor), null, new IdentityQueryFilter(DatabaseDescriptor.createMain(false), databaseDescriptor.getTracing(), databaseDescriptor.getDBConfig()), 1000);
        ...
        Scrubber scrubber = new Scrubber(cfs, sstable, databaseDescriptor.getCompactionManager(), false, false, DatabaseDescriptor.createMain(false), databaseDescriptor.getDBConfig(), databaseDescriptor.getStorageService());
        try
        {
            scrubber.scrub();
        ...
        scrubber = new Scrubber(cfs, sstable, databaseDescriptor.getCompactionManager(), true, false, DatabaseDescriptor.createMain(false), databaseDescriptor.getDBConfig(), databaseDescriptor.getStorageService());
        scrubber.scrub();
        scrubber.close();
    ...
    public void testScrubOutOfOrder() throws Exception
    {
        ...
        */
        ...
        CFMetaData metadata = databaseDescriptor.getSchema().getCFMetaData(desc.ksname, desc.cfname);

        try
        {
            DatabaseDescriptor.createMain(false).getSSTableReaderFactory().open(desc, metadata);
        ...
        SSTableReader sstable = DatabaseDescriptor.createMain(false).getSSTableReaderFactory().openNoValidation(desc, components, metadata);

        Scrubber scrubber = new Scrubber(cfs, sstable, databaseDescriptor.getCompactionManager(), false, true, DatabaseDescriptor.createMain(false), databaseDescriptor.getDBConfig(), databaseDescriptor.getStorageService());
        scrubber.scrub();
