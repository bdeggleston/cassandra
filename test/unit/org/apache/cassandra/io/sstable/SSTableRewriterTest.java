...
public class SSTableRewriterTest extends SSTableWriterTestBase
{
    ...
    public static Set<SSTableReader> writeFiles(ColumnFamilyStore cfs, int fileCount, int partitionCount, int cellCount)
    {
        ...
        for (int f = 0 ; f < fileCount ; f++)
        {
            ...
            try (SSTableTxnWriter writer = SSTableTxnWriter.create(cfs, desc, 0, 0, null, false, MutationIdMetadata.NONE, new SerializationHeader(true, cfs.metadata(), cfs.metadata().regularAndStaticColumns(), EncodingStats.NO_STATS)))
            {
