...
public class SSTableMetadataTest
{
    ...
    public static void defineSchema() throws Exception
    {
        ...
        SchemaLoader.createKeyspace(KEYSPACE1,
                                    ...
                                    databaseDescriptor.getCFMetaDataFactory().denseCFMetaData(KEYSPACE1, CF_STANDARDCOMPOSITE2, compositeMaxMin),
    ...
    public void testTrackMaxDeletionTime()
    {
        ...
        Mutation rm = databaseDescriptor.getMutationFactory().create(KEYSPACE1, Util.dk("longttl", databaseDescriptor).getKey());
        ...
        rm = databaseDescriptor.getMutationFactory().create(KEYSPACE1, Util.dk("longttl2", databaseDescriptor).getKey());
        rm.add("Standard1", cellname("col"),
               ByteBufferUtil.EMPTY_BYTE_BUFFER,
               timestamp,
               20000);
    ...
    public void testWithDeletes() throws ExecutionException, InterruptedException
    {
        ...
        long timestamp = System.currentTimeMillis();
        DecoratedKey key = Util.dk("deletetest", databaseDescriptor);
        Mutation rm = databaseDescriptor.getMutationFactory().create(KEYSPACE1, key.getKey());
        for (int i = 0; i<5; i++)
    ...
    public void trackMaxMinColNames() throws CharacterCodingException, ExecutionException, InterruptedException
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            DecoratedKey key = Util.dk("row"+j, databaseDescriptor);
            Mutation rm = databaseDescriptor.getMutationFactory().create(KEYSPACE1, key.getKey());
        ...
        DecoratedKey key = Util.dk("row2", databaseDescriptor);
        Mutation rm = databaseDescriptor.getMutationFactory().create(KEYSPACE1, key.getKey());
    ...
    {
        ColumnFamilyStore cfs = databaseDescriptor.getKeyspaceManager().open(KEYSPACE1).getColumnFamilyStore("Counter1");
        ...
        databaseDescriptor.getMutationFactory().create(Util.dk("k", databaseDescriptor).getKey(), cells).applyUnsafe();
        cfs.forceBlockingFlush();
