...
public class LongCompactionsTest
{
    ...
    protected void testCompaction(int sstableCount, int rowsPerSSTable, int colsPerRow) throws Exception
    {
        ...
        new CompactionTask(store, sstables, gcBefore, false, DatabaseDescriptor.createMain(false, false), databaseDescriptor.getSystemKeyspace(), databaseDescriptor.getDBConfig(), databaseDescriptor.getStorageService()).execute(null);
    ...
    public void testStandardColumnCompactions() throws IOException, ExecutionException, InterruptedException
    {
        ...
        assertEquals(inserted.size(), Util.getRangeSlice(cfs, DatabaseDescriptor.createMain(false, false), databaseDescriptor.getTracing()).size());
